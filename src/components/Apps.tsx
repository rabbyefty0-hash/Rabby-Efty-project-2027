...
export function Apps({ onNavigate, isVpnConnected, setIsVpnConnected }: AppsProps) {
  ...
  const renderAppIcon = (app: any, idx: number) => (
    <motion.div
      key={app.id}
      ...
      {isLoading ? (
        ...
        <>
          <button
            ...
            className="flex flex-col items-center space-y-1.5 group w-full p-1 active:opacity-70"
          >
            <div className={`${getIconSizeClass()} ${getIconShapeClass()} ${app.bg} flex items-center justify-center ios-icon transition-all duration-300 group-hover:scale-105 group-active:scale-95 shadow-sm`}>
  ...
  return (
    <div className="flex-1 overflow-y-auto px-6 pt-12 pb-40 relative z-10 custom-scrollbar h-full w-full">
      <div className="max-w-md mx-auto relative">
        {/* Sticky Search Bar */}
        <div className="sticky top-0 z-50 pb-6 pt-4">
        ...
        {searchQuery ? (
          <div className="grid grid-cols-4 gap-x-4 gap-y-8 content-start px-2 pb-8">
            ...
            {categories.map((category, catIdx) => {
              ...
              return (
                <motion.div 
                  ...
                  className="bg-white/5 border border-white/10 rounded-[2rem] p-5 backdrop-blur-md shadow-lg"
                >
                  <h3 className="text-white/90 font-semibold mb-5 px-2 text-lg tracking-tight">{category.name}</h3>
